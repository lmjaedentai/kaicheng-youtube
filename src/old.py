...
def error_handling(func):
    async def err_inner(arg,**kwargs): #kwargs mean any other args avai
        try:
...
def insertquery(x=None,query=None):
    ...
    task[n] = query
    thread = threading.Thread(target= downloadyoutube, args=(task[n],n))#QQ yt
    thread.start()       

def downloadyoutube(arg,iid):
    ...
    with YoutubeDL({'format': 'bestaudio','outtmpl': thepath+'\%(title)s.mp3','--windows-filenames': True,'nocheckcertificate': True,'ignoreerrors': False,'logtostderr': False,'default_search': 'auto','source_address': '0.0.0.0'}) as ydl:
        try:
            ...
            try:
                ...
                displayhelp('error [query extraction fail]',f'{e}\n\nWe apologize for the inconvenience caused.')
                return table.item(iid,tags=('red',))
                raise e
        else:
            try:
                ...
                displayhelp('error [link extraction fail]',f'{e}\n\nWe apologize for the inconvenience caused.')
                return table.item(iid,tags=('red'))
        #renaming file
        try:
            if playlistmode.get() == False:
            ...
            displayhelp('error [file saving fail]',f'{e}\n\nWe apologize for the inconvenience caused.')
            return table.item(iid,tags=('red'))
        else:
            table.item(iid,tags=('green'))
            pass

def displayhelp(title='About Us', content=None):
    helpui = tk.Toplevel(root)
    helpui.title("KaiCheng YT downloader")
    helpui.geometry('400x600')
    helpui.attributes('-alpha', 0.94)
    ttk.LabelFrame(helpui, text=title, width=350, height=200).place(x=20, y=20)
    if content == None:
        ...
        ttk.Label(helpui, text=content, font=("Consolas", 9), foreground = "white", wraplength=320).place(x=40,y=70,width=320)
